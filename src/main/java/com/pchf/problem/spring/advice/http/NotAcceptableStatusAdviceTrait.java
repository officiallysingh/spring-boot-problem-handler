...
public interface NotAcceptableStatusAdviceTrait<T, R> extends BaseNotAcceptableAdviceTrait<T, R> {
  ...
  @ExceptionHandler
  default R handleMediaTypeNotAcceptable(final NotAcceptableStatusException exception, final T request) {
    return toProblem(exception, request, GeneralErrorKey.INTERNAL_SERVER_ERROR, HttpStatus.NOT_ACCEPTABLE);
  }
}
