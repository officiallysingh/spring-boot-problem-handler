...
public class ProblemUtils {
	...
	public static ThrowableProblem toProblem(final Throwable throwable) {
		...
		ThrowableProblem problem = Problem.of(status).detail(throwable.getMessage())
				.build();
	...
	public static ResponseStatus resolveResponseStatus(final Throwable type) {
		@Nullable
		final ResponseStatus candidate = findMergedAnnotation(type.getClass(),
	...
	public static StackTraceElement[] createStackTrace(final Throwable throwable) {
		...
		if (cause == null
				|| !ProblemBeanRegistry.problemProperties().isCauseChainsEnabled()) {
			...
			final int length = current.length
					- Lists.lengthOfTrailingPartialSubList(asList(next), asList(current));
