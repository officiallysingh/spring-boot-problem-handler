 ...
 */
public abstract class ThrowableProblem extends RuntimeException
		implements Problem, Exceptional {
	...
	protected ThrowableProblem(@Nullable final ThrowableProblem cause) {
		...
		final Collection<StackTraceElement> stackTrace = StackTraceProcessor.COMPOUND
				.process(asList(getStackTrace()));
		setStackTrace(stackTrace.toArray(new StackTraceElement[0]));
	}

	@Override
	public String getDetail() {
		return Stream.of(getCode(), getTitle()).filter(Objects::nonNull)
				.collect(joining(": "));
	}
