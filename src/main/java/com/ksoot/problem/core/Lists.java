...
final class Lists {
  ...
  /**
   * Returns the length of the longest trailing partial sublist of the target list within the
   * specified source list, or 0 if there is no such occurrence. More formally, returns the length
   * <tt>i</tt> such that {@code source.subList(source.size() - i,
   * source.size()).equals(target.subList(target.size() - i, target.size()))}, or 0 if there is no
   * such index.
   *
   * @param source the list in which to search for the longest trailing partial sublist of
   *     <tt>target</tt>.
