...
public class SpringWebErrorResponseBuilder
	...
	@Override
	public ResponseEntity<ProblemDetail> buildResponse(final Throwable throwable,
			final NativeWebRequest request, final HttpStatus status,
		...
		Optional<ResponseEntity<ProblemDetail>> responseEntity = negotiate(request)
				.map(contentType -> ResponseEntity.status(status).headers(headers)
						.contentType(contentType).body(problemDetail));
	...
	private ResponseEntity<ProblemDetail> postProcess(
			final ResponseEntity<ProblemDetail> errorResponse,
			final NativeWebRequest request) {
		return errorResponse;
	}

	private ResponseEntity<ProblemDetail> fallback(final NativeWebRequest request,
	...
	public HttpMethod requestMethod(final NativeWebRequest request) {
		return HttpMethod
				.valueOf(request.getNativeRequest(HttpServletRequest.class).getMethod());
	}
}
