...
public interface OpenApiValidationAdviceTrait<T, R>
	...
	default R handleInvalidRequest(final InvalidRequestException exception,
			final T request) {
		final List<ViolationVM> violations = handleValidationReport(
				exception.getValidationReport(), exception);
	...
	@ExceptionHandler
	default R handleInvalidResponse(final InvalidResponseException exception,
			final T request) {
		final List<ViolationVM> violations = handleValidationReport(
				exception.getValidationReport(), exception);
		Map<String, Object> parameters = new LinkedHashMap<>(4);
	...
	default List<ViolationVM> handleValidationReport(
		...
		return validationReport.getMessages().stream()
				...
				.flatMap(message -> handleValidationReportMessage(message, exception)
						.stream())
				.toList();
	}

	default List<ViolationVM> handleValidationReportMessage(final Message message,
			final Throwable exception) {

		HttpStatus status = defaultConstraintViolationStatus();
		String propertyPath = message.getContext().flatMap(MessageContext::getParameter)
				.map(Parameter::getName).orElse("");
		...
		return errorPropertyKeys.stream().map(propertyKeys -> {
			String[] codeCodes = Arrays.stream(propertyKeys)
					.map(errorKey -> ProblemConstant.CODE_CODE_PREFIX
							+ GeneralErrorKey.OPEN_API_VIOLATION + ProblemConstant.DOT
							+ errorKey)
					.toArray(String[]::new);
			String[] messageCodes = Arrays.stream(propertyKeys)
					.map(errorKey -> ProblemConstant.DETAIL_CODE_PREFIX
							+ GeneralErrorKey.OPEN_API_VIOLATION + ProblemConstant.DOT
							+ errorKey)
					.toArray(String[]::new);

			ProblemMessageSourceResolver codeResolver = ProblemMessageSourceResolver
					.of(codeCodes, "" + status.value());
			ProblemMessageSourceResolver messageResolver = ProblemMessageSourceResolver
					.of(messageCodes, message.getMessage());
	...
	default List<String[]> deriveOpenApiValidationErrorKeys(final Message message) {
		String propertyKey = message.getContext().flatMap(MessageContext::getParameter)
				.map(Parameter::getName).orElse("");
		Method requestMetod = message.getContext()
				.flatMap(MessageContext::getRequestMethod).orElse(null);
		String requestPath = message.getContext().flatMap(MessageContext::getRequestPath)
		...
		if (isEmpty(propertyKey)) {
			if (requestMetod == null || requestMetod == Method.GET) {
				...
				try {
					...
					// messageText.substring(i1 + 2, j1)
					// : String.join(DOT, Arrays.stream(messageText.substring(i2 + 2,
					// j2).split("/"))
					// .filter(part -> !StringUtils.isNumeric(part)).toList());
					String objectPath = (i2 == -1 || j2 == -1) ? ""
							: String.join(ProblemConstant.DOT, Arrays
									.stream(messageText.substring(i2 + 2, j2).split("/"))
									.filter(part -> !StringUtils.isNumeric(part))
									.toList());
					if (i1 == -1) {
						List<String[]> errorKeys = new ArrayList<>(1);
						errorKeys.add(new String[] {
								message.getKey() + requestPath + ProblemConstant.DOT
										+ requestMetod.name().toLowerCase()
										+ ProblemConstant.DOT + objectPath,
								message.getKey() + requestPath + ProblemConstant.DOT
										+ objectPath,
								message.getKey() + ProblemConstant.DOT + objectPath,
								objectPath });
						return errorKeys;
					}
					List<String> properties = Arrays
							.asList(messageText.substring(i1 + 1, j1).replaceAll("\"", "")
									.replace("[", "").replace("]", "").split(","))
							.stream().toList();
					String objectPathPart = StringUtils.isNotBlank(objectPath)
							? ProblemConstant.DOT + objectPath + ProblemConstant.DOT
							: ProblemConstant.DOT;
					return properties.stream().map(property -> new String[] {
							message.getKey() + requestPath + ProblemConstant.DOT
									+ requestMetod.name().toLowerCase() + objectPathPart
									+ property,
							message.getKey() + requestPath + objectPathPart + property,
							message.getKey() + objectPathPart + property }).toList();
				}
				catch (Exception e) {
					List<String[]> errorKeys = new ArrayList<>(1);
					errorKeys.add(new String[] {
			...
			errorKeys.add(new String[] {
					message.getKey() + requestPath + ProblemConstant.DOT
							+ requestMetod.name().toLowerCase() + ProblemConstant.DOT
							+ propertyKey,
					message.getKey() + requestPath + ProblemConstant.DOT + propertyKey,
					message.getKey() + ProblemConstant.DOT + propertyKey });
			return errorKeys;
		}
	}
}
