...
/**
 * The request was a valid request, but the server is refusing to respond to it. The user
 * might be logged in but does not have the necessary permissions for the resource.
 */
public interface AccessDeniedAdviceTrait<T, R> extends AdviceTrait<T, R> {

	@ExceptionHandler
	default R handleAccessDeniedException(final AccessDeniedException exception,
			final T request) {
		Problem problem = toProblem(exception, HttpStatus.FORBIDDEN,
				ProblemMessageSourceResolver.of(
						ProblemConstant.DETAIL_CODE_PREFIX
								+ GeneralErrorKey.SECURITY_ACCESS_DENIED,
