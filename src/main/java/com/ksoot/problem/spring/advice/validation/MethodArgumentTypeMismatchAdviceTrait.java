...
public interface MethodArgumentTypeMismatchAdviceTrait<T, R>
	...
	default R handleMethodArgumentTypeMismatch(
		...
		String errorKey = exception.getParameter().getContainingClass().getSimpleName()
				+ ProblemConstant.DOT + exception.getParameter().getMethod().getName()
				+ ProblemConstant.DOT + parameterName;

		String codeCode = ProblemConstant.CODE_CODE_PREFIX + GeneralErrorKey.TYPE_MISMATCH
				+ ProblemConstant.DOT + errorKey;
		String titleCode = ProblemConstant.TITLE_CODE_PREFIX
				+ GeneralErrorKey.TYPE_MISMATCH + ProblemConstant.DOT + errorKey;
		String detailCode = ProblemConstant.DETAIL_CODE_PREFIX
				+ GeneralErrorKey.TYPE_MISMATCH + ProblemConstant.DOT + errorKey;
		...
		Problem problem = toProblem(exception,
				...
				ProblemMessageSourceResolver.of(detailCode,
